...
var sharkObj = function () {
	...
	this.testSharkAndPlayer = function () {

		for (var i = 1; i > 0; i--)//判断玩家序列帧个数
		{
			for (var j = this.keyframe.length - 1; j >= 0; j--)//判断漂浮物关键帧序列帧总个数
			{
				var hit = hitTestPoint(this.keyframe[j].x, this.keyframe[j].y, sharkWidth, sharkHeight, playerX, playerY);
				if (hit) {
				}
			}
		}
...
sharkObj.prototype.draw = function () {
	...
	this.image = addImg("../img/huaji4.png");
...
sharkObj.prototype.createShark = function (number)		//海藻数量
{
	this.num = number;
	for (var i = 0; i < this.num; i++) {
		for (var pn = 1; pn <= 1; pn++) {
			// var item = addImg("plankter/plankter (" + pn + ").png");
			var item = addImg("../img/huaji4.png");
...
sharkObj.prototype.draw2 = function () {
	...
	for (var i = 0; i < this.keyframe.length; i++) {
		...
		if (item.x > cw - sharkWidth) {
			item.speedX = -item.speedX;
			item.x = cw - sharkWidth;
		}
		if (item.y < 0) {
		...
		if (item.y > ch - sharkWidth) {
			item.speedY = -item.speedY;
			item.y = ch - sharkWidth;
		}
	}
